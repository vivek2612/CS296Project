...
int main(int argc, char** argv)
{
  ...
  float32 time_taken_step = 0.0f;
  float32 time_taken_loop = 0.0f;
  float32 time_taken_collisions = 0.0f;
  float32 time_taken_velocity = 0.0f;
  float32 time_taken_position = 0.0f;
  entry = sim;
  ...
  dummy = settimeofday(&myTime,NULL);
  dummy = gettimeofday(&start,NULL);
  for (long i = 0; i < iterations;i++) {
  	test->step(&settings);
  	time_taken_step += ((test->get_world())->GetProfile()).step;
  	time_taken_velocity += ((test->get_world())->GetProfile()).solveVelocity;
  	time_taken_collisions += ((test->get_world())->GetProfile()).collide;
  	time_taken_position += ((test->get_world())->GetProfile()).solvePosition;
  }
